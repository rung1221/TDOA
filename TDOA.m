...
% 设置地面反射系数 (0到1之间，0表示完全吸收，1表示完全反射)
% 典型的地面反射系数取决于地面材质：
% - 水泥/沥青: 0.95
% - 草地: 0.7-0.8
% - 松软土壤: 0.5-0.6
ground_reflection_coeff = 0.95; % 假设地面是水泥/沥青
...
for i = 1:num_mics
    ...
    % === 添加地面反射路径 ===
    % 镜像声源法：无人机关于地面的镜像点(假设地面在z=0平面)
    mirror_drone_pos = [drone_pos(1), drone_pos(2), -drone_pos(3)];
    
    % 计算从镜像声源到麦克风的向量
    mirror_direction_vec = mic_positions(i,:) - mirror_drone_pos;
    reflect_dist = norm(mirror_direction_vec);
    
    % 反射路径的单位方向向量
    if reflect_dist > 0
        mirror_unit_direction = mirror_direction_vec / reflect_dist;
    else
        mirror_unit_direction = [0, 0, 0];
    end
    
    % 计算反射路径上风对声速的影响
    mirror_wind_proj = dot(wind_velocity, mirror_unit_direction);
    mirror_c_effective = c + mirror_wind_proj;
    
    % 反射声波的延迟时间
    reflect_delay_time = reflect_dist / mirror_c_effective;
    reflect_delay_samples = round(reflect_delay_time * fs);
    
    % 计算入射角(声波与地面法线的夹角)
    incident_vector = [drone_pos(1) - mirror_drone_pos(1), drone_pos(2) - mirror_drone_pos(2), drone_pos(3) - mirror_drone_pos(3)];
    ground_normal = [0, 0, 1]; % 地面法向量指向上
    
    cos_incident_angle = dot(incident_vector, ground_normal) / (norm(incident_vector) * norm(ground_normal));
    incident_angle = acos(cos_incident_angle);
    
    % 声波反射系数随入射角变化(简化模型，接近垂直入射时反射更强)
    angle_factor = 0.5 + 0.5 * cos(incident_angle); % 在0.5到1之间变化
    effective_reflection = ground_reflection_coeff * angle_factor;
    
    % 反射声波衰减
    reflect_dist_attenuation = 1 / reflect_dist;
    reflect_air_absorption = exp(-0.002 * density_factor * reflect_dist);
    reflect_attenuation = reflect_dist_attenuation * reflect_air_absorption * effective_reflection;
    
    % 添加反射声波到接收信号
    if reflect_delay_samples < length(t)
        delayed_signal(reflect_delay_samples+1:end) = delayed_signal(reflect_delay_samples+1:end) + drone_signal(1:end-reflect_delay_samples) * reflect_attenuation;
    end
...
for i = [1:ref_mic-1, ref_mic+1:num_mics]
    ...
    max_lag = min(round(0.1 * fs), floor(length(t)/2)-1);
        ...
        % 3. PHAT加权 (相位变换) - 强调相位信息，忽略幅值信息
        Gxy_phat = Gxy ./ (abs(Gxy) + eps); % 添加eps避免除零
        ...
        % 6. 确定延迟范围
        lags = -floor(N/2):floor((N-1)/2);
        
        % 7. 限制在最大延迟范围内
        valid_range = (lags >= -max_lag) & (lags <= max_lag);
        valid_lags = lags(valid_range);
        valid_gcc = gcc(valid_range);
        
        % 8. 找最大值位置
        [~, max_idx] = max(abs(valid_gcc));
...
%% 分析地面反射对定位精度的影响
% 创建不同地面反射系数的测试
reflection_coeffs = [0, 0.3, 0.5, 0.8, 1.0]; % 从无反射到完全反射
num_coeffs = length(reflection_coeffs);
reflection_errors = zeros(num_coeffs, 1);

fprintf('\n===== 地面反射影响分析 =====\n');
fprintf('反射系数\t定位误差(m)\t高度误差(m)\n');
fprintf('------------------------\n');

for coef_idx = 1:num_coeffs
    % 设置当前反射系数
    current_coef = reflection_coeffs(coef_idx);
    
    % 使用当前反射系数生成信号
    test_mic_signals = generate_signals_with_reflection(drone_pos, mic_positions, drone_signal, fs, c, wind_velocity, air_density, current_coef);
    
    % 使用带反射的信号计算TDOA
    test_tdoa = calculate_tdoa(test_mic_signals, ref_mic, fs, t);
    test_distance_diffs = test_tdoa * c;
    
    % 使用标准定位方法估计位置
    test_error_func = @(pos) location_error(pos, mic_positions, ref_mic, test_distance_diffs, c, wind_velocity);
    ...
    for i = 1:num_starts
        ...
        if cost < best_cost
            best_cost = cost;
            refl_estimated_pos = pos;
        end
    end
    
    % 计算误差
    pos_error = refl_estimated_pos - drone_pos;
    reflection_errors(coef_idx) = norm(pos_error);
    
    % 输出结果
    fprintf('%.2f\t\t%.2f\t\t%.2f\n', current_coef, norm(pos_error), abs(pos_error(3)));
end

% 绘制反射系数对定位精度的影响
figure('Position', [100, 100, 600, 400]);
bar(reflection_coeffs, reflection_errors);
xlabel('地面反射系数');
ylabel('定位误差 (m)');
title('地面反射对声源定位精度的影响');
grid on;
xticks(reflection_coeffs);
xticklabels({'0 (无反射)', '0.3 (弱反射)', '0.5 (中等)', '0.8 (强反射)', '1.0 (完全反射)'});
xtickangle(45);

fprintf('\n总结: 地面反射系数为%.1f时定位误差最小。\n', ...
        reflection_coeffs(reflection_errors == min(reflection_errors)));
...
% 绘制镜像声源
h_mirror = plot3(mirror_drone_pos(1), mirror_drone_pos(2), mirror_drone_pos(3), 'ro', ...
                'MarkerSize', 10, 'MarkerFaceColor', [1 0.7 0.7], 'DisplayName', '镜像声源');
legend_handles = [legend_handles; h_mirror];
legend_labels{end+1} = '镜像声源(地面反射)';

% 绘制虚线表示地面反射路径 - 只选择几个代表性麦克风以避免图像过于复杂
highlight_mics = [1, 5, 9, 10, 14, 18]; % 选择底层、中层和顶层的代表性麦克风
for i = 1:length(highlight_mics)
    mic_idx = highlight_mics(i);
    % 从镜像声源到麦克风的反射路径
    plot3([mirror_drone_pos(1), mic_positions(mic_idx,1)], ...
          [mirror_drone_pos(2), mic_positions(mic_idx,2)], ...
          [mirror_drone_pos(3), mic_positions(mic_idx,3)], ...
          'r:', 'LineWidth', 1);
          
    % 可选：添加从真实声源到镜像点的辅助线(表明镜像关系)
    if i == 1 % 只为第一个麦克风添加这条线以避免混乱
        plot3([drone_pos(1), drone_pos(1)], ...
              [drone_pos(2), drone_pos(2)], ...
              [drone_pos(3), mirror_drone_pos(3)], ...
              'k:', 'LineWidth', 1);
    end
end

% 添加地面反射说明文本
text(drone_pos(1)+0.5, drone_pos(2)+0.5, 0, '地面反射面', ...
     'FontSize', 10, 'Color', 'k', 'BackgroundColor', [0.9 0.9 0.9]);

% 添加标签
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Z (m)');
title(sprintf('麦克风阵列声源定位 - 环境因素及地面反射影响分析 (温度: %.1f °C)', temperature));
...
fprintf('5. 地面反射影响: 反射系数为%.1f时误差最小\n', reflection_coeffs(reflection_errors == min(reflection_errors)));
...
function mic_signals = generate_signals(drone_pos, mic_positions, drone_signal, fs, c, wind_velocity, air_density)
    ...
    for i = 1:num_mics
        ...
        % 计算入射角(声波与地面法线的夹角)
        incident_vector = [drone_pos(1) - mirror_drone_pos(1), 
                          drone_pos(2) - mirror_drone_pos(2), 
                          drone_pos(3) - mirror_drone_pos(3)];
        ground_normal = [0, 0, 1];
        ...
        % 声波反射系数随入射角变化
        angle_factor = 0.5 + 0.5 * cos(incident_angle);
        effective_reflection = ground_reflection_coeff * angle_factor;
        ...
        % 添加反射声波
        if reflect_delay_samples < length(drone_signal)
            delayed_signal(reflect_delay_samples+1:end) = delayed_signal(reflect_delay_samples+1:end) + drone_signal(1:end-reflect_delay_samples) * reflect_attenuation;
        end
...
% 带有可配置反射系数的信号生成函数
function mic_signals = generate_signals_with_reflection(drone_pos, mic_positions, drone_signal, fs, c, wind_velocity, air_density, reflection_coeff)
    num_mics = size(mic_positions, 1);
    mic_signals = zeros(num_mics, length(drone_signal));
    ...
    for i = 1:num_mics
        ...
        % === 反射路径 (使用传入的反射系数) ===
        if reflection_coeff > 0  % 如果反射系数为0，则跳过计算反射
            mirror_drone_pos = [drone_pos(1), drone_pos(2), -drone_pos(3)];
            mirror_direction_vec = mic_positions(i,:) - mirror_drone_pos;
            reflect_dist = norm(mirror_direction_vec);
            ...
            angle_factor = 0.5 + 0.5 * cos(incident_angle);
            effective_reflection = reflection_coeff * angle_factor;
...
% 计算TDOA (使用GCC-PHAT方法)这里是给算误差的时候使用的函数
function tdoa_estimates = calculate_tdoa(mic_signals, ref_mic, fs, t)
    ...
    for i = [1:ref_mic-1, ref_mic+1:num_mics]
        ...
        % 设置安全的最大延迟窗口
        max_lag = min(round(0.1 * fs), floor(length(sig_i)/2)-1);
